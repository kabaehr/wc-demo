...
class SeatElement extends LitElement {
	...
	render() {
		return html`
			<div class="seat-container ${this.seat.hasReservation ? 'reserved' : ''} ${ this.isSelected ? 'selected' : '' }" @click="${this.emitClickEvent}"></div>
		`;
	}
}

customElements.define('seat-element', SeatElement);
//${JSON.stringify(this.seat.info.number)}
//herz oder disabled bild
